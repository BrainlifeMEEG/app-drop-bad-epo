...
print(f'DEBUG: loading epochs from: {data_file}')
epochs = mne.read_epochs(data_file, verbose=False)
print(f'Loaded {len(epochs)} epochs')

# === DEBUG: state of the epochs object as loaded ===
print('=' * 60)
print('DEBUG: epochs as loaded')
print('=' * 60)
print(f'  len(epochs)        = {len(epochs)}')
print(f'  epochs.selection[:10] = {epochs.selection[:10]}')
print(f'  epochs.selection[-5:] = {epochs.selection[-5:]}')
print(f'  len(epochs.drop_log) = {len(epochs.drop_log)}')
print(f'  number of already-dropped entries in drop_log = '
      f'{sum(1 for r in epochs.drop_log if r)}')
print('=' * 60)
...
if already_bad:
    ...
    print(f'DEBUG: count of already-bad indices in drop_log = {len(already_bad)}')
...
if config.get('events') and config['events'] != 'None':
    print(f'DEBUG: events file param IS set -> {config["events"]!r}')
    try:
        with open(config['events']) as f:
            raw_contents = f.read()
        print(f'DEBUG: raw contents of events file (first 500 chars):')
        print(repr(raw_contents[:500]))
        # Turn todrop1 into a list of strings and remove leading/trailing whitespace
        todrop1 = raw_contents.split(',')
        ...
        print(f'DEBUG: todrop1 (from file) = {todrop1}')
    ...
    print(f'DEBUG: events file param NOT set (value = {config.get("events")!r})')

# Parse drop parameter if provided
todrop2 = []
if config.get('drop') and config['drop'] != 'None':
    print(f'DEBUG: drop param IS set -> {config["drop"]!r}')
    try:
        ...
        print(f'DEBUG: todrop2 (from drop param) = {todrop2}')
    ...
    print(f'DEBUG: drop param NOT set (value = {config.get("drop")!r})')

# Create union of all drops
todrop = sorted(list(set(todrop1) | set(todrop2)))

# === DEBUG: final list to drop ===
print('=' * 60)
print('DEBUG: final indices that will be passed to epochs.drop()')
print('=' * 60)
print(f'  todrop1 (from file)       = {todrop1}')
print(f'  todrop2 (from drop param) = {todrop2}')
print(f'  todrop (union, sorted)    = {todrop}')
print(f'  len(todrop)               = {len(todrop)}')
print('=' * 60)
...
if todrop:
    ...
    # === DEBUG: which epoch(s) are we actually about to drop? ===
    print('DEBUG: about to call epochs.drop(todrop)')
    print(f'  len(epochs) BEFORE drop  = {len(epochs)}')
    print(f'  epochs.selection at indices {todrop} = '
          f'{[int(epochs.selection[i]) for i in todrop if i < len(epochs)]}')

    print(f'Dropping {len(todrop)} epochs: {todrop}')
    epochs.drop(todrop)

    print(f'  len(epochs) AFTER drop   = {len(epochs)}')
